...
def calculate_wide_format_stats(data_root, methods, datasets, distribution_type="iid",
                                save_filename="final_wide_stats.csv", max_step=5):
    """
    功能：
...
def calculate_task0_retention_stats(data_root, methods, datasets, distribution_type="iid",
...
def merge_arg(data_root:str, data_type: str):
    """
    读取指定类型(iid/ood)的csv文件，对不同数据集的Task Mean按方法求平均，
    并保存为新的合并后csv文件。
    """
    # 1. 校验输入参数
    if data_type not in ["iid", "ood"]:
        raise ValueError("输入参数必须是 'iid' 或 'ood'")

    input_filename = f"{data_root}/{data_type}_arg.csv"
    output_filename = f"{data_root}/{data_type}_merge_arg.csv"
    ...
    # 4. 筛选需要的列 (只需要 Method 和包含 _Mean 的列)
    mean_cols = [col for col in df.columns if col.endswith('_Mean')]
    columns_to_keep = ['Method'] + mean_cols
    df_filtered = df[columns_to_keep]

    # 5. 按 Method 分组求均值
    # sort=False 可以保持 Method 第一次出现的顺序（比如 PLAID 在最前）
    # pandas 的 .mean() 会自动跳过 NaN 空值，保障计算准确
    df_merged = df_filtered.groupby('Method', as_index=False, sort=False).mean()

    # 6. 保存为新的 CSV 文件
    # float_format='%.4f' 可以让输出的数值保持4位小数，看起来更整洁 (可根据需要去掉)
    df_merged.to_csv(output_filename, index=False, float_format='%.4f')

    print(f"处理完成！不同数据集已按方法求取平均，结果保存在: {output_filename}")
...
def merge_arg_task0(data_root: str, data_type: str):
    """
    读取指定类型(iid/ood)的 task0 csv文件，对不同数据集的 Step Mean 按方法求平均，
    并保存为新的合并后csv文件。
    """
    # 1. 校验输入参数
    if data_type not in ["iid", "ood"]:
    ...
    # 假设你的输入文件命名规则为 iid_arg_task0.csv
    input_filename = f"{data_root}/{data_type}_arg_task0.csv"
    output_filename = f"{data_root}/{data_type}_merge_arg_task0.csv"
    ...
    # 4. 筛选需要的列 (专门筛选 Method 以及 Step0_Mean 到 Step5_Mean)
    # 这样写可以精准捕获以 Step 开头且以 _Mean 结尾的列，忽略 _Std 等其他无关列
    mean_cols = [col for col in df.columns if col.startswith('Step') and col.endswith('_Mean')]

    # 如果没找到任何符合条件的列，给出提示
    if not mean_cols:
        print(f"警告: 在 {input_filename} 中没有找到任何类似 'StepX_Mean' 的列！")
        return
    ...
    # 5. 按 Method 分组求均值
    # sort=False 可以保持 Method 第一次出现的顺序
    # pandas 的 .mean() 会自动跳过 NaN 空值，保障计算准确
    df_merged = df_filtered.groupby('Method', as_index=False, sort=False).mean()

    # 6. 保存为新的 CSV 文件
    # float_format='%.4f' 可以让输出的数值保持4位小数，看起来更整洁
    df_merged.to_csv(output_filename, index=False, float_format='%.4f')

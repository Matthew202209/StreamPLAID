...
from models.mrhp_ab_no_repair.residual_embedding import ResidualEmbeddings
...
class ResidualCodec:
    ...
    def __init__(self, config, centroids, avg_residual=None, bucket_cutoffs=None, bucket_weights=None, use_gpu = False):
        self.use_gpu = False
        ...
        self.dim, self.nbits = config["model_config"]["compression_dim"], config["run_config"]["nbits"]
        ...
        self.rank = config["run_config"]["rank"]
    ...
    def try_load_torch_extensions(cls, use_gpu):
        ...
        print(f"Loading packbits_cpp extension (set COLBERT_LOAD_TORCH_EXTENSION_VERBOSE=True for more info)...")
        packbits_cpp = load(
            name="packbits_cpp",
            sources=[
                os.path.join(
    ...
    @classmethod
    def load(cls, config , index_path, use_gpu=False):
    ...
    def compress_new_cluster(self,embs, centroids_, codes_, cents_id_2_idx):
        pass

    def compress(self, embs, centroids_, codes_):
        if not isinstance(embs, torch.Tensor):
            embs = torch.tensor(embs)

            # 保证 centroids_ 是 Tensor
        if not isinstance(centroids_, torch.Tensor):
            centroids_ = torch.tensor(centroids_)

            # 保证 codes_ 是 Tensor
        if not isinstance(codes_, torch.Tensor):
            codes_ = torch.tensor(codes_)

        codes, residuals = [], []
        # 统一按 embs 的 batch 大小切分
        batch_size = 1 << 18
        for i in range(0, embs.shape[0], batch_size):
            sl = slice(i, i + batch_size)
            batch_embs = embs[sl]  # (B, d)
            batch_codes = codes_[sl]  # (B,)
            batch_cents = centroids_[batch_codes]  # (B, d) 或 (B,)
            res = batch_embs - batch_cents  # 残差
            codes.append(batch_codes)
            residuals.append(self.binarize(res))

        return ResidualCodec.Embeddings(
    ...
    def compress_into_codes(self, embs, hdrc_cluster):
        """
        ...
        for batch in embs.split(bsize):
            indices, _ = hdrc_cluster.structure_query(batch)
            codes.append(torch.tensor(indices))
        return torch.cat(codes)

    def lookup_centroids(self, codes, hdrc_cluster):
        """
        ...
        for batch in codes.split(1 << 20):
            ...
            batch_centroids = hdrc_cluster.get_centroids_by_idx(batch)
            centroids.append(torch.tensor(batch_centroids, dtype=torch.float32))
            del batch

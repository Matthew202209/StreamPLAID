...
class ResidualEmbeddings:
    ...
    def add_compress_embs(self, add_embs):
        self.codes = torch.cat((self.codes, add_embs.codes), dim=0)
        self.residuals = torch.cat((self.residuals, add_embs.residuals), dim=0)

    def remove_compress_embs(self, remove_indices):
        mask = torch.ones(self.codes.size(0), dtype=torch.bool)
        mask[remove_indices] = False
        self.codes = self.codes[mask]

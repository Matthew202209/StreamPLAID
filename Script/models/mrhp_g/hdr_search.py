...
class LayerData:
    ...
    stats_dist_sq:np.ndarray
    init_num_vecs: np.float32
    init_all_dist_sq:np.float32
...
class HDRSearch:
    ...
    def set_faiss_index(self):
        ...
        valid_ids = np.arange(len(centroids))[non_zero_mask]
        base_index = faiss.IndexFlatIP(dim)
        index = faiss.IndexIDMap(base_index)
        index.add_with_ids(valid_centroids, valid_ids.astype(np.int64))
        self.faiss_centroids_index = index
    ...
    def _cal_num_centers(self, n_points):
        ...
        theo_num = int(2 ** np.floor(np.log2(np.sqrt(16*n_points))))
    ...
    def _build_layer_core(self, vecs: np.ndarray, global_ids: np.ndarray,
        ...
        # 5. 筛选覆盖点
        min_dists = 2 * (1.0 - np.minimum(max_scores, 1.0))
        min_dists_sqrt = np.sqrt(min_dists)

        hit_mask = min_dists_sqrt <= current_radius
        hit_vecs = vecs[hit_mask]
        hit_ids = global_ids[hit_mask]
        hit_center_indices = assigned_local_centers[hit_mask]
        hit_min_dists = min_dists[hit_mask]

        # layer_sum_dist = np.sum(hit_min_dists)
        miss_vecs = vecs[~hit_mask]
        miss_ids = global_ids[~hit_mask]
        actual_n_centers = S_i.shape[0]

        # 6. 向量化统计 count, sum, sum_sq, sum_dist_sq
        stats_count = np.zeros(actual_n_centers, dtype=np.int32)
        np.add.at(stats_count, hit_center_indices, 1)

        stats_dist_sq = np.zeros(actual_n_centers, dtype=np.float32)
        np.add.at(stats_dist_sq, hit_center_indices, hit_min_dists)
        ...
        init_num_vecs = np.sum(stats_count)
        init_all_dist_sq = np.sum(stats_dist_sq)
        # 10. 打包 LayerData
        layer_data = LayerData(
            ...
            stats_dist_sq = stats_dist_sq,
            init_num_vecs= init_num_vecs,
            init_all_dist_sq= init_all_dist_sq,
    ...
    def _force_assign_to_layer(self, vecs: np.ndarray, global_ids: np.ndarray):
        """
        ...
        # 1. 确定中心数
        total_centers_needed = self._cal_num_centers(self.num_data if self.num_data > 0 else n_samples)
        n_centers = self._geometric_allocation(total_centers_needed, self.l_max-1) # -1 因为是从 0 开始的层级
        n_centers = min(n_centers, n_samples)
        random_indices = np.random.choice(n_samples, n_centers, replace=False)
        S_random_init = vecs[random_indices]
        ...
        max_scores = all_max_scores
        assigned_local_centers = all_assigned_centers
        min_dists_sq = 2 * (1.0 - np.minimum(max_scores, 1.0))

        # 1. 计算最后一层的量化误差总和，供后续增量更新时的成本估计使用


        # 2. 计算最后一层的聚类中心在全局的 ID（从 global_center_id_counter 开始分配）
        actual_n_centers = S_i.shape[0]
        center_global_ids = np.arange(self.global_center_id_counter,
                                      self.global_center_id_counter + actual_n_centers, dtype=np.int32)
        ...
        # 3.计算分到每个中心的点数、向量和、向量平方和（用于后续增量更新时的成本估计）

        # =================== 分到每个中心的点数 =======================
        stats_count = np.zeros(actual_n_centers, dtype=np.int32)
        np.add.at(stats_count, assigned_local_centers, 1)

        # =================== 分到每个中心的向量和 =======================
        stats_sum = np.zeros((actual_n_centers, self.vec_dim), dtype=np.float32)
        np.add.at(stats_sum, (assigned_local_centers,), vecs)


        # =================== 分到每个中心的向量平方和 =======================
        stats_sq = np.zeros((actual_n_centers, self.vec_dim), dtype=np.float32)
        np.add.at(stats_sq, (assigned_local_centers,), vecs ** 2)

        stats_dist_sq = np.zeros(actual_n_centers, dtype=np.float32)
        np.add.at(stats_dist_sq, assigned_local_centers, min_dists_sq)


        # 4. 构建倒排索引 (Inverted Index)，记录每个中心对应的全局点 ID 列表
        inverted_index = defaultdict(list)
        unique_centers = np.unique(assigned_local_centers)
        for cid in unique_centers:
            inverted_index[cid] = global_ids[assigned_local_centers == cid].tolist()
        ...
        # 6. 合并到一起
        layer_data = LayerData(
            layer_index=self.l_max-1,
            centroids=S_i,
            radius=float('inf'),  # 兜底层不设半径限制
            center_global_ids=center_global_ids,
            stats_count=stats_count,
            stats_sum_vec=stats_sum,
            stats_sum_sq=stats_sq,
    ...
    def fit(self, U):
        ...
        for i in range(self.l_max-1): # 聚类到倒数第二层，最后一层直接强制嵌入
            print(f"处理层 {i}...")
        ...
        if len(current_vecs) > 0:
            if len(self.layers) > 0:
                ...
                print(f"有 {len(current_vecs)} 个点，落到了最后一层 {self.layers[-1].layer_index}")
                layer_data = self._force_assign_to_layer(current_vecs, current_ids)
                self.layers.append(layer_data)
            else:
    ...
    def add(self, x: np.ndarray):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            # if idx == num_layers - 1:
            #     min_dist_sq, best_local_cids = self._compute_nearest_centers_last_layer(q_vecs, layer.centroids)
            # else:
            centroids_faiss = self.faiss_centroids_list[idx]
            min_dist_sq, best_local_cids = self._compute_nearest_centers(q_vecs, centroids_faiss)
            ...
            valid_dist_sq = min_dist_sq[hit_mask]

            # 原子更新层级统计信息
            if idx == num_layers - 1:
                self._batch_update_layer_stats(
                    layer, valid_cids, valid_vecs,
                    valid_gids, valid_dist_sq, affected_clusters
                )
            else:
                self._batch_update_layer_stats_only_iv(layer, valid_cids, valid_vecs,
                                                       valid_gids, valid_dist_sq, affected_clusters)
    ...
    def _compute_nearest_centers_last_layer(self, vecs, centroids):
        """
        ...
        #================ 检查已经被软删除的向量 ==================
        dim = centroids.shape[1]
        is_zero_vector = ~np.any(centroids != 0, axis=1)
        has_zero_vectors = np.any(is_zero_vector)

        if not has_zero_vectors:
            # --- 情况 A: 数据很干净，直接用常规索引 ---
            print("Check: 未发现全零向量，使用 Standard IndexFlatIP")
            index = faiss.IndexFlatIP(dim)
            index.add(centroids)
        else:
            # --- 情况 B: 包含全零向量，启用 ID 映射逻辑 ---
            print(f"Check: 发现 {np.sum(is_zero_vector)} 个全零向量，启用 IndexIDMap 过滤")

            # 提取非零向量及其原始索引
            non_zero_mask = ~is_zero_vector
            valid_centroids = centroids[non_zero_mask]
    ...
    def _compute_nearest_centers(self, vecs, index):
    ...
    def _batch_update_layer_stats(self, layer, cids, vecs, gids, dist_sq, affected_tracker):
        """
        [原子操作] 批量更新某层的统计量和索引
        """
        # 1. Numpy 快速统计量更新
        ...
        np.add.at(layer.stats_dist_sq, cids, dist_sq)
    ...
    def _batch_update_layer_stats_only_iv(self,layer, cids, vecs, gids, dist_sq, affected_tracker):
        np.add.at(layer.stats_dist_sq, cids, dist_sq)
        np.add.at(layer.stats_count, cids, 1)
        group_dict = defaultdict(list)
        for i, cid in enumerate(cids):
    ...
    def _check_split_conditions(self, affected_clusters):
        ...
        def cal_G(layer):
            """
            :param layer: 拿到层级数据，计算量化误差的估计量eps_est, 当前层的量化误差 eps_this
            :return:
            """
            n_0 = layer.init_num_vecs  #
            n = np.sum(layer.stats_count)
            epsilon_0 = layer.init_all_dist_sq / n_0
            epsilon_this = np.sum(layer.stats_dist_sq) / n
            epsilon_est = calculate_epsilon_prime(epsilon_0, 128, n_0, n)
            return calculate_Go(epsilon_this, epsilon_est)
    ...
    def _pack_layer_data(self, layer_idx, centroids, radius, hit_vecs, hit_ids, hit_min_dists ,hit_center_indices):
        """
        ...
        # 新的聚类数据计算出来的init_num_vecs, init_all_dist_sq
        # init_num_vecs = np.sum(stats_count)
        # init_all_dist_sq = np.sum(stats_dist_sq)
            ...
            for check_idx in range(start_layer_idx, max_layer_idx + 1):
                ...
                if ratio >= self.beta or is_last_layer:
                    ...
                    if not is_last_layer:
                        ...
                        hit_dist_sq = min_dist_sq[hit_mask]
                        ...
                        # 生成 Patch
                        patch = self._pack_layer_data(check_idx, centroids,
                                                      target_radius, hit_vecs,
                                                      hit_ids, hit_dist_sq, hit_cids)
                    ...
                    # 无论 Ratio 是否达标，必须全盘接收
                        ...
                        patch = self._pack_layer_data(check_idx, centroids, target_radius,
                                                      hit_vecs, hit_ids, hit_dist_sq, hit_cids)
    ...
    def update_clusters_with_dict(self, update_reqs: List[Tuple[int, List[int]]]):
        """
        [最终优化版] 增量重聚类：全链路统一逻辑
        不再区分"中间层"和"最后一层"，所有层级共享一次 KMeans 和距离计算。
        """
                ...
                if ratio >= self.beta or is_last_layer:
                    ...
                    if not is_last_layer:
                        ...
                        patch = self._pack_layer_data(check_idx, centroids,
                                                      target_radius, hit_vecs,
                                                      hit_ids, hit_dist_sq, hit_cids)
                        self._merge_update_patch_with_dict(check_idx, patch, update_vecs_2_cent_dict)
        ...
        return update_vecs_2_cent_dict

    def update_clusters_with_dict_and_cell_ids(self, update_reqs: List[Tuple[int, List[int]]]):
        """
        [最终优化版] 增量重聚类：全链路统一逻辑
        不再区分"中间层"和"最后一层"，所有层级共享一次 KMeans 和距离计算。
        """
        if not update_reqs: return
        global_cancel_cells_ids = []
        print(f"--- 触发增量重聚类: 处理 {sum(len(ids) for _, ids in update_reqs)} 个簇 ---")
        # 0 构建一个字典存重聚类的向量与新聚类中心
        update_vecs_2_cent_dict = {}
        # 1. 提取数据 & 软删除 (保持不变)
        ...
        for l_idx, c_idxs in req_map.items():
            layer = self.layers[l_idx]
            c_idxs_arr = np.array(c_idxs, dtype=np.int32)
            global_cancel_cells_ids += list(layer.center_global_ids[c_idxs_arr])
            for cid in c_idxs:
                p_ids = layer.inverted_index.get(cid, [])
                if not p_ids: continue
                p_ids_arr = np.array(p_ids, dtype=np.int32)
        ...
        return update_vecs_2_cent_dict, global_cancel_cells_ids
    ...
    def _soft_delete_clusters(self, layer_idx: int, local_cids: List[int]):
        ...
        layer.stats_dist_sq[cids_arr] = 0
    ...
    def _merge_update_patch(self, layer_idx: int, patch: LayerData):
        ...
        base.stats_dist_sq = np.concatenate([base.stats_dist_sq, patch.stats_dist_sq])

        # 2. 因为触发了重聚类，所有初始量化误差要重新计算一遍
        base.init_num_vecs = np.sum(base.stats_count)
        base.init_all_dist_sq = np.sum(base.stats_dist_sq)
    ...
    def _merge_update_patch_with_dict(self, layer_idx: int, patch: LayerData,  update_vecs_2_cent_dict):
        """
        将新生成的 Patch 追加到层尾，并更新全局索引。
        """
        base = self.layers[layer_idx]
        ...
        for local_cid, gids in patch.inverted_index.items():
            base.inverted_index[local_cid + offset] = gids
            global_cid =  base.center_global_ids[local_cid + offset]
            update_vecs_2_cent_dict.update({gid: global_cid for gid in gids})
    ...
    def get_point_to_cluster_map(self) -> np.ndarray:
        ...
        for layer in self.layers:
            if not layer.inverted_index:
                continue

            # 用于收集这一层所有的点 ID 和对应的簇 ID
            layer_p_ids = []
            layer_c_ids = []
            ...
            for local_cid, point_ids in layer.inverted_index.items():
                ...
                # 核心优化：用 Python 原生列表批量收集，避免在循环中创建 NumPy 数组
                layer_p_ids.extend(point_ids)
                # 将 global_cid 重复 len(point_ids) 次，与 point_ids 一一对应
                layer_c_ids.extend([global_cid] * len(point_ids))

            # 每层只做一次 NumPy 数组转换和高级索引赋值
            if layer_p_ids:
                p_ids_arr = np.array(layer_p_ids, dtype=np.int32)
                c_ids_arr = np.array(layer_c_ids, dtype=np.int32)
                assignments[p_ids_arr] = c_ids_arr

        return assignments

    # def get_point_to_cluster_map(self) -> np.ndarray:
    #     """
    #     获取全量数据的簇分配结果（极致向量化，无惧簇数量增长）。
    #     """
    #     assignments = np.full(self.num_data, -1, dtype=np.int32)
    #
    #     for layer in self.layers:
    ...
    #             continue
    #
    #         # 1. 过滤空簇，并将 keys (local_cid) 和 values (point_ids list) 分离
    #         valid_items = [(cid, pids) for cid, pids in layer.inverted_index.items() if pids]
    #         if not valid_items:
    #             continue
    #
    #         local_cids, all_point_ids_lists = zip(*valid_items)
    #
    #         # 2. 【极速展平】：使用 C 底层的 itertools.chain 把嵌套的 list 瞬间打平
    #         # 这一步把所有文档 ID 融合成一根长面条
    #         flat_pids = list(itertools.chain.from_iterable(all_point_ids_lists))
    #
    #         # 3. 提取对应的全局中心 ID 列表
    #         # （假设 center_global_ids 是字典或列表，这里用列表推导式提取，速度极快）
    #         global_cids = [layer.center_global_ids[cid] for cid in local_cids]
    #
    #         # 4. 获取每个簇里有多少个文档
    #         lengths = [len(pids) for pids in all_point_ids_lists]
    #
    #         # 5. 【C 级内存复制】：把每个全局中心 ID 按照对应的长度复制
    #         # 例如：把 [A, B] 按长度 [2, 3] 复制成 [A, A, B, B, B]
    #         repeated_cids = np.repeat(global_cids, lengths)
    #
    #         # 6. 【终极一击】：全程唯一一次 NumPy 赋值！
    #         # 无论有几十万个簇，底层的 C 语言也只需执行一次批量内存写入
    #         assignments[flat_pids] = repeated_cids
    #
    #     return assignments
...
def calculate_epsilon_prime(epsilon_0, d, n_0, n):
    """
    根据公式计算 epsilon_prime
    ...
    # 错误检查：防止除以零
    if n == 0:
        raise ValueError("参数 n 不能为 0")
    if d == 0:
        raise ValueError("参数 d (根指数) 不能为 0")

    # 计算公式: epsilon_0 * ((n_0 / n) 的 (1/d) 次方)
    epsilon_prime = epsilon_0 * ((n_0 / n) ** (1 / d))

    return epsilon_prime


def calculate_Go(epsilon, epsilon_prime):
    """
    根据公式计算 Go

    参数:
    epsilon: 变量 ε
    epsilon_prime: 变量 ε' (分母，不能为0)
    ...
    # 错误检查：防止除以零
    if epsilon_prime == 0:
        raise ValueError("分母 epsilon_prime (ε') 不能为 0")

    # 计算公式: |ε - ε'| / ε'
    # abs() 函数用于计算绝对值
    Go = abs(epsilon - epsilon_prime) / epsilon_prime

...
from models.mrhp_ab_flat.base_model import HF_ColBERT
from models.mrhp_ab_flat.util import _split_into_batches, _insert_prefix_token
...
class QueryTokenizer():
    def __init__(self, config, verbose: int = 3):
        ...
        self.config = config
        self.query_maxlen = config["run_config"]["max_query_len"]
        self.background_maxlen = 512 - self.query_maxlen + 1

        self.Q_marker_token, self.Q_marker_token_id = config["model_config"]["query_token"], self.tok.convert_tokens_to_ids(
            config["model_config"]["query_token_id"])
    ...
    def tensorize(self, batch_text, bsize=None, context=None, full_length_search=False):
        ...
        obj = self.tok(batch_text, padding='max_length', truncation=True,
                       return_tensors='pt', max_length=(max_length - 1)).to(self.device)
        ...
        if self.config["model_config"]["attend_to_mask_tokens"]:
